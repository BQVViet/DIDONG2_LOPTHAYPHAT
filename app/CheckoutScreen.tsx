...
export default function CheckoutScreen() {
  ...
  // --- 2. LOAD DATA & FIX LỖI UNDEFINED ---
  useEffect(() => {
    const loadData = async () => {
      try {
        let rawData = "";
        if (params.cartData) {
          rawData = params.cartData as string;
          await AsyncStorage.setItem('temp_checkout_items', rawData);
        } else {
          rawData = await AsyncStorage.getItem('temp_checkout_items') || "[]";
        }
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
            ...
            <View key={item.id || index} style={styles.productRow}>
              ...
              <View style={styles.productInfo}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.productQty}>Số lượng: {item.finalQty}</Text>
                </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.paymentGrid}>
            {[
              { id: 'cod', label: 'Tiền mặt', icon: 'cash-outline' },
              { id: 'bank', label: 'Ngân hàng', icon: 'swap-horizontal-outline' },
              { id: 'wallet', label: 'Ví điện tử', icon: 'wallet-outline' },
            ].map((method) => (
              <TouchableOpacity
                key={method.id}
        ...
        <TouchableOpacity style={styles.promoCard}>
          ...
          <View style={{ flex: 1, marginLeft: 12 }}>
            <Text style={styles.promoTitle}>Voucher ưu đãi</Text>
            <Text style={styles.promoSub}>{discount > 0 ? `Đã áp dụng giảm ${safeFormatPrice(discount)}` : 'Sử dụng mã để tiết kiệm hơn'}</Text>
          </View>
          <Ionicons name="chevron-forward" size={18} color="#94A3B8" />
        </TouchableOpacity>
      ...
      <View style={styles.footer}>
        <View style={styles.summaryContainer}>
          ...
          {discount > 0 && (
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Giảm giá</Text>
              <Text style={styles.discountText}>-{safeFormatPrice(discount)}</Text>
            </View>
          )}
          <View style={styles.totalRow}>
            <View>
...
const styles = StyleSheet.create({
  ...
  userName: { fontSize: 15, fontWeight: '700', color: '#0F172A', marginBottom: 4 },
  addressText: { fontSize: 13, color: '#64748B', lineHeight: 20 },
  productRow: { flexDirection: 'row', gap: 12, marginBottom: 12, alignItems: 'center' },
  productImg: { width: 70, height: 70, borderRadius: 12, backgroundColor: '#F1F5F9' },
  productInfo: { flex: 1, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  productName: { fontSize: 14, fontWeight: '700', color: '#0F172A', flexShrink: 1 },
  variantText: { fontSize: 12, color: '#64748B', marginTop: 2, fontStyle: 'italic' },
  productQty: { fontSize: 12, color: '#94A3B8', marginTop: 2 },
  productPrice: { color: '#0F172A', fontWeight: '800', fontSize: 15 },
  paymentGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  paymentItem: { flex: 1, minWidth: '28%', paddingVertical: 14, borderRadius: 16, borderWidth: 2, borderColor: '#F1F5F9', alignItems: 'center', gap: 6 },
